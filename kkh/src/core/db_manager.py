...
def create_tables():
    ...
    # AI 상호작용 로깅 테이블 (대시보드 데이터용)
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS ai_interaction_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id TEXT NOT NULL,
        timestamp DATETIME NOT NULL,
        interaction_type TEXT NOT NULL, -- e.g., 'function_call_success', 'function_call_fail', 'slot_filling'
        details TEXT, -- JSON string with more info, like function_name or error message
        FOREIGN KEY (user_id) REFERENCES users (user_id)
    );
    """)
...
# --- Logging Functions for Dashboard ---

def log_interaction(user_id: str, interaction_type: str, details: dict = None):
    """AI 상호작용을 데이터베이스에 기록합니다."""
    now = datetime.utcnow()
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO ai_interaction_log (user_id, timestamp, interaction_type, details) VALUES (?, ?, ?, ?)",
        (user_id, now, interaction_type, details_json)
    )
    conn.commit()
    conn.close()
