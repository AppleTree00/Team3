...
# ui.md '6.2-A' 및 '6.2-D'에 따른 일정 등록 확인 UI
if st.session_state.pending_action:
    action_type = st.session_state.pending_action['action']
    ...
    with st.chat_message("assistant"):
        ...
        # IMPROVEMENT-01: 과거 날짜 등록 시 UI 분기 (qa.md TC-NLU-06)
        if action_type == "confirm_past_creation":
            header_text = "⚠️ 과거 시점의 일정입니다."
            message_text = "선택하신 날짜는 과거입니다. 기록을 위해 등록하시겠습니까?"
            confirm_button_text = "네, 등록합니다"
            cancel_button_text = "아니요, 취소합니다"
        else: # "confirm_creation"
            header_text = "✨ 새로운 일정을 등록할까요?"
            message_text = ""
        ...
        col1, col2, _ = st.columns([1.2, 1.2, 3]) # 버튼 텍스트 길이를 고려하여 비율 조정
        with col1:
            if st.button(confirm_button_text, use_container_width=True):
        ...
        with col2:
            if st.button(cancel_button_text, use_container_width=True):
...
if prompt := st.chat_input("내일 오후 3시에 개발팀 주간 회의 잡아줘"):
    if st.session_state.pending_action:
        ...
        if isinstance(response, dict):
            if response.get("action") in ["confirm_creation", "confirm_past_creation"]:
